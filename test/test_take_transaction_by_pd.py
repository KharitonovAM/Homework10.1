...
@patch(
    "csv.DictReader",
    return_value=[{"data": "123", "name": "MyName"}, {"data": 121, "name": "Alice"}],
)
def test_take_transactions_from_csv(mock_csv_DictReader):
    open("test.csv", "w")
    assert take_transactions_from_csv("test.csv") == [
        {"data": "123", "name": "MyName"},
        {"data": 121, "name": "Alice"},
    ]
...
def test_patch_test_take_transactions_from_exel():
    ...
    assert take_transactions_from_exel("my_exel.xlsx") == [
        {"name": "Ivan", "family": "Бунин"},
        {"name": "Olga", "family": "Кортункова"},
    ]
