...
def filter_by_state(list_of_dict: list[dict[Any]], state: str = "EXECUTED") -> list[dict[Any]]:
    """Функция возвращает новый список словарей, содержащий только те словари, у которых ключ
    state соответствует указанному значению"""
    return [item for item in list_of_dict if item["state"] == state]


def sort_by_date(list_of_dict: list[dict[Any]], date_reverse: bool = False) -> list[dict[Any]]:
